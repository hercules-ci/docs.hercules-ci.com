...
The agent currently only supports https://cachix.org/[Cachix] binary caches.

Note that binary cache configuration affects the whole system's Nix configuration.
If you're deploying without using the Nix module system, refer to <<deploy-with-manual-config-file>>.

To write the file, see <<binary-caches-json>>.
