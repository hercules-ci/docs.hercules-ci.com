...
On https://cachix.org[Cachix] you can create a binary cache. Once you complete the process, gather the keys into a `binary-caches.json` file, replacing all placeholders:
...
NOTE: For more detail, see <<binary-caches-json,The `binary-caches.json` format>> in the Reference.
