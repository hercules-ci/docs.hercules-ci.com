...
= Known issues
 ...
 - The identity token must be manually deleted when rekeying the agent token. https://github.com/hercules-ci/hercules-ci-agent/issues/8[issue #8]
  - Agent can only build for a single github account (user / organization)
...
1. Save the example https://nixos.org/nixops/[NixOps] network as `network.nix` in a new directory:
+
[source,nix]
----
...
----
// TODO frontend
2. Selecting an account in the https://hercules-ci.com[dashboard]
3. Click the "Generate agent token" button near the bottom of the page
   This produces a private token that should be protected like a password.
4. Copy the bare token and save it to a plain text file `agent-token.key` in the same directory. Protect the `agent-token.key` file like a password.

5. Add a physical network specification (`deployment.targetEnv`, etc), adapt the example concurrency configuration as appropriate for your hardware and deploy the network. NixOps can deploy to
...
6. To deploy the agent run the shell commands:
+
[source,bash]
----
...
----
+
The agent will start working as soon as the deployment has succeeded and work is pending. See link:#add-a-repository[Add a repository].
...
= Step 3. Add a repository
 ...
 1. Click the "Install hercules-ci application" button near the bottom of the https://hercules-ci.com[dashboard] and follow the steps.
 2. Write a `default.nix` in the root of your repository.
+
...
Hercules CI behaves similarly to `nix-build`. `default.nix` can provide multiple derivations by returning an attribute set. Nested attribute sets can be traversed by calling Nixpkgs' `pkgs.recurseIntoAttrs`.
 ...
 3. Test it locally by running the command
+
[source,bash]
----
myrepo/$ NIX_PATH= nix-build --no-out-link
----
 4. Commit and push, so Hercules CI will pick it up.

= FAQ / How To / Troubleshooting
